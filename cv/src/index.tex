...
\usepackage{ifxetex}
\ifxetex
  % XeTeX handles UTF-8 and fonts natively
\else
...
\info{%
    ...
    \faicon{envelope}   & pfyu@google.com
}

\bibliography{cv}

\addtocategory{workshop}{no:hotos17}

\addtocategory{poster}{salus:michai18,salus:sysml18}

\addtocategory{papers}{fluid:mlsys21,salus:mlsys20,fb:ieeemicro,orloj:arxiv22}

\addtocategory{phd}{peifeng:dissertation}
...
\begin{itemize}
    ...
    \newline \hfill \textit{Tech Lead since 2025}
    \begin{itemize}
        \item \textbf{Hardware Management Framework:} Led a team of 5 to architect a unified, composable hardware management framework for a large-scale cluster manager, reducing engineering integration overhead for next-generation ML accelerators by an estimated 50\%, by decoupling monolithic legacy code into modular, configuration-driven components.
        \item \textbf{Hardware Platform Onboarding:} Designed and implemented node-level cluster manager support for next-generation ML accelerators (including Trillium and Ironwood), enabling seamless production integration and rapid qualification for the latest hardware generations.
        \item \textbf{ML Workload Virtualization:} Spearheaded the fleet-wide migration of massive-scale ML training workloads to secure, hardware-accelerated virtual machines, improving multi-tenant isolation while maintaining strict performance SLAs, by optimizing node-level memory allocation and secure device passthrough policies.
        \item \textbf{Resource Scheduling Optimization:} Led resource scheduling optimization initiatives, significantly improving fleet-wide compute utilization and eliminating hardware stranding, by dynamically aligning auxiliary compute and memory allocations with primary ML accelerator requests.
        \item \textbf{Reliability \& Security:} Enhanced fleet-wide hardware reliability and platform security by engineering automated preflight diagnostics and migrating legacy health-check execution paths into a safe-by-default, unprivileged architecture, mitigating critical vulnerabilities and reducing workload downtime.
        \item \textbf{Team Leadership:} As Tech Lead, directed long-term infrastructure strategy and cross-functional engineering initiatives, accelerating overall team velocity and incident resolution, by authoring architectural roadmaps, establishing rigorous code-quality standards, and actively mentoring software engineers.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Identify optimization opportunities and give improvement suggestions via automated emails.
    \end{itemize}
...
\begin{itemize}
    \item Self-Hosted Kubernetes Cluster (Homelab) \hfill \textit{From 2023 to present}
    \begin{itemize}
        \item Designed and provisioned a multi-node Kubernetes cluster (\textbf{k3s}) spanning across a WAN, managed entirely via \textbf{Pulumi} as Infrastructure as Code (IaC).
        \item Implemented robust internal security with per-pod \textbf{mTLS} using \textbf{cert-manager} and automated service reloading upon certificate renewal.
        \item Deployed and managed distributed storage using \textbf{JuiceFS} and centralized authentication via \textbf{Authelia} to secure internal web services.
        \item Configured secure ingress and reverse proxying using \textbf{Traefik} with automated certificate provisioning.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Side projects including a rust unicode string truncation library, network traffic capture and analyze, torrent list migration between uTorrent and qBittorrent.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Successful Google Summer of Code project. Link: \url{https://summerofcode.withgoogle.com/archive/2016/projects/6014826014834688/}
